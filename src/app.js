...
(function () {
  ...
  // ----- Generator -----------------------------------------------------------
  // Pure function from state → Map<filename, content>. Same input always
  // produces the same output (modulo build_info.built_at). Used both by
  // PreviewScreen (rendering the bundle without packing) and, in a later
  // milestone, by Bundler to ZIP-pack the result.
  ...
  const Generator = (function () {
    ...
    function _question(phase, qid) {
      return ((phase && phase.questions) || []).find(function (q) { return q.id === qid; });
    }
    ...
    function generateMasterPrompt(state, taxonomy) {
      const slug = _projectSlug(state);
      const pitch = (((state.phases.vision || {}).answers || {}).pitch || {}).value || "(untitled)";
      const qb = state.quality_bar || "personal";
      const visionAnswers = (state.phases.vision || {}).answers || {};
      ...
      for (const [name, desc] of FILE_INDEX) {
        const pad = ".".repeat(Math.max(2, 40 - name.length));
        lines.push("- `" + name + "` " + pad + " " + desc);
      }
      lines.push("");
      const buildInfo = Data.buildInfo || {};
      lines.push("Generated by prompt-wizard v" + (buildInfo.wizard_version || "dev") + " on " + new Date().toISOString().split("T")[0] + ".");
    ...
    function generateMetaTechStack(state, taxonomy) {
      const ffAnswers = (state.phases.form_factor || {}).answers || {};
      ...
      const stacksDoc = Data.techStacks || { form_factors: {} };
      const lines = [];
    ...
    function generateMetaPrerequisites(state) {
      const ffAnswers = (state.phases.form_factor || {}).answers || {};
      const primary = (ffAnswers.primary_factor && ffAnswers.primary_factor.value) || "";
      const stacksDoc = Data.techStacks || { form_factors: {} };
      const prereqDoc = Data.prerequisites || { tools: {} };
      const ff = (stacksDoc.form_factors || {})[primary];
      // Collect tool ids from the primary form factor's first stack as a starting set.
      const toolIds = new Set(["git"]);
      if (ff && ff.stacks && ff.stacks.length) {
        for (const id of (ff.stacks[0].prerequisites || [])) toolIds.add(id);
      }
      const lines = [];
      ...
      for (const phase of phases) {
        ...
        if (ps.stale_since && !ps.user_acknowledged_stale) {
          items.push({
            phase: phase,
            kind: "stale",
            message: ps.stale_reason || "Upstream answer changed."
          });
        }
        for (const q of (phase.questions || [])) {
          const a = _answer(state, phase.id, q.id);
    ...
    function generateAll(state, taxonomy) {
      const t = taxonomy || Data.questionTaxonomy;
      const out = {};
      out["README.md"] = generateReadme(state);
      out["PROMPT.md"] = generateMasterPrompt(state, t);
      // 15 phase docs
      const phases = (t && t.phases) || [];
      for (const p of phases) {
        const num = String(p.number).padStart(2, "0");
        const fname = "docs/" + num + "-" + p.id.replace(/_/g, "-") + ".md";
        out[fname] = generatePhaseDoc(state, t, p.id);
      }
      // meta synthesis
      out["meta/tech-stack.md"]      = generateMetaTechStack(state, t);
      out["meta/non-functional.md"]  = generateMetaNonFunctional(state);
      out["meta/architecture.md"]    = generateMetaArchitecture(state);
      out["meta/prerequisites.md"]   = generateMetaPrerequisites(state);
      out["meta/build-plan.md"]      = generateMetaBuildPlan(state);
      out["meta/test-plan.md"]       = generateMetaTestPlan(state);
      out["meta/open-questions.md"]  = generateMetaOpenQuestions(state, t);
      out["meta/out-of-scope.md"]    = generateMetaOutOfScope(state);
      // notes + raw state
      out["notes/user-notes.md"]     = generateUserNotes(state, t);
